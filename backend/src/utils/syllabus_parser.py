...
class SyllabusParser:
    ...
    def parse(self) -> Dict[str, Optional[str]]:
        ...
        numbering_td = self.soup.find(
            "span", class_="lesson_plan_subheading", string=lambda x: x is not None and "(科目ナンバリング)" in x
        )
        if numbering_td:
        ...
        # 英訳: 「(英 訳)」の行を探す
        eng_td = self.soup.find("span", class_="lesson_plan_subheading", string=lambda x: x is not None and "(英 訳)" in x)
        if eng_td:
        ...
        teacher_info_table = self.soup.find("td", class_="lesson_plan_sell", string=lambda x: isinstance(x, str) and "(所属部局)" in x)
        ...
        # "(配当学年)"が記載されている行を探す
        year_td = self.soup.find("span", class_="lesson_plan_subheading", string=lambda x: "(配当学年)" in x if x else False)
        if year_td:
        ...
        day_time_td = self.soup.find("span", class_="lesson_plan_subheading", string=lambda x: False if x is None else "(曜時限)" in x)
        if day_time_td:
        ...
        # 使用言語
        lang_td = self.soup.find("span", class_="lesson_plan_subheading", string=lambda x: isinstance(x, str) and "(使用言語)" in x)
        if lang_td:
        ...
        textbook_sub = self.soup.find("span", class_="lesson_plan_subheading", string=lambda x: bool(x) and "(教科書)" in x)
        if textbook_sub:
        ...
        # 参考書等
        ref_sub = self.soup.find("span", class_="lesson_plan_subheading", string=lambda x: isinstance(x, str) and "(参考書等)" in x)
        if ref_sub:
